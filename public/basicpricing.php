...
if (isset($_SESSION['user_id']) && isset($_POST['start_basic'])) {
    $user_id = $_SESSION['user_id'];

    // Ellenőrizzük, hogy a felhasználó vállalkozó-e
    $result = $mysqli->query("SELECT role, basic_start FROM users WHERE id = $user_id");
    $row = $result->fetch_assoc();

    if ($row['role'] === 'provider') {
        if (empty($row['basic_start'])) {
            $now = date('Y-m-d H:i:s');
            $mysqli->query("UPDATE users SET basic_start = '$now' WHERE id = $user_id");
        }
...
<header>
    <h1>Basic csomag</h1>
    <p>Csatlakozz vállalkozóként, és élvezd a szolgáltatás előnyeit havi 2000 Ft-ért!</p>
</header>

<div class="package-card">
    <h2>Basic csomag</h2>
    <p class="price">2000 Ft / hónap</p>
    <p>Teljes hozzáférés a szolgáltatásokhoz vállalkozók számára.</p>
    ...
    <?php if (isset($_SESSION['user_id'])): ?>
        ...
        <?php else: ?>
            <?php if ($subscription_started): ?>
                <p style="color:#10b981; font-weight:600;">A Basic csomag előfizetésed aktív!</p>
            <?php else: ?>
                <form method="POST">
                    <button type="submit" name="start_basic" class="btn btn-primary btn-block">Előfizetés indítása</button>
                </form>
            <?php endif; ?>
        <?php endif; ?>
    <?php else: ?>
...
<div class="info-section">
    ...
    <p>Havi 2000 Ft-ért hozzáférsz minden funkcióhoz, és vállalkozóként teljesen profi élményben részesülsz. Ha még nem rendelkezel vállalkozói fiókkal, regisztrálj most.</p>
</div>
